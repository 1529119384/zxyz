...
describe('files API', () => {
  ...
  it('应调用 GET 获取下载链接', async () => {
    vi.mocked(request.get).mockResolvedValue({
      code: 1,
      msg: 'ok',
      data: { downloadUrl: 'https://cdn/file.pdf' },
    })
  ...
  it('应调用 PATCH 重命名文件', async () => {
    vi.mocked(request.patch).mockResolvedValue({ code: 1, msg: 'ok', data: null })
  ...
  it('应调用 PATCH 移动文件并返回信封', async () => {
    vi.mocked(request.patch).mockResolvedValue({ code: 1, msg: 'ok', data: { successCount: 2 } })
  ...
  it('应调用 POST 复制文件并返回信封', async () => {
    vi.mocked(request.post).mockResolvedValue({ code: 1, msg: 'ok', data: { successCount: 1 } })
  ...
  describe('fetchFileList 参数与数据兜底', () => {
    it('带 spaceType/projectId/sortOrder 时全部进入 params', async () => {
      vi.mocked(request.get).mockResolvedValue({ code: 1, msg: 'ok', data: { list: [], total: 0 } })
    ...
    it('无 page/pageSize 时不带上分页参数', async () => {
      vi.mocked(request.get).mockResolvedValue({ code: 1, msg: 'ok', data: { list: [] } })
      await fetchFileList(1)
      expect(request.get).toHaveBeenCalledWith('/api/files', {
        params: { parentId: 1 },
        signal: undefined,
    ...
    it('data 为直接数组（无 list 字段）时也能映射', async () => {
      vi.mocked(request.get).mockResolvedValue({ code: 1, msg: 'ok', data: [{ id: 9 }] })
  ...
  describe('moveFiles / copyFiles 可选参数', () => {
    it('moveFiles 带 spaceType/projectId 时透传', async () => {
      vi.mocked(request.patch).mockResolvedValue({ code: 1, msg: 'ok', data: {} })
  ...
  describe('上传链路', () => {
    ...
    it('directUpload 组装 FormData 并用 multipart 头', async () => {
      ...
      const [url, rawFormData, config] = vi.mocked(request.post).mock.calls[0]
      const formData = /** @type {FormData} */ (rawFormData)
    ...
    it('directUpload 省略可选参数时只带 file', async () => {
      ...
      const formData = /** @type {FormData} */ (vi.mocked(request.post).mock.calls[0][1])
      expect(formData.get('file')).toBe(file)
  ...
  describe('fetchStorageUsage', () => {
    ...
    it('带 spaceType/teamId/projectId 时全部进入 params', async () => {
      vi.mocked(request.get).mockResolvedValue({ code: 1, msg: 'ok', data: { usedStorage: 100 } })
