...
describe('im API', () => {
  ...
  it('应调用 POST 标记已读', async () => {
    vi.mocked(imRequest.post).mockResolvedValue({})
  ...
  it('应调用 GET 检查 IM 服务健康状态', async () => {
    vi.mocked(imRequest.get).mockResolvedValue({ data: { status: 'UP' } })
    await fetchImHealth()
  ...
  it('应调用 POST 创建单聊会话', async () => {
    vi.mocked(imRequest.post).mockResolvedValue({ data: {} })
  ...
  it('应调用 GET 获取系统通知未读数', async () => {
    vi.mocked(imRequest.get).mockResolvedValue({ data: { unreadCount: 3 } })
  ...
  it('应调用 PATCH 标记单条系统通知已读', async () => {
    vi.mocked(imRequest.patch).mockResolvedValue({})
