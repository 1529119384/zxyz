...
class FileControllerTest {
    ...
    void getUploadSign_delegatesToFileUploadPort() {
        ...
        when(fileUploadPort.getUploadSign("test.txt", null, 1L)).thenReturn(signInfo);

        UploadInfo result = fileUploadPort.getUploadSign("test.txt", null, 1L);

        assertNotNull(result);
        assertEquals("https://oss.example.com/put", result.getUploadUrl());
        verify(fileUploadPort).getUploadSign("test.txt", null, 1L);
    }
