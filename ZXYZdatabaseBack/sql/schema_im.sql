-- ============================================================================
-- zxyz-im-service database schema v2.0
-- 指绣云章IM服务数据库 — 全面规范化重构
--
-- 使用方式：通过 rebuild_all.sql 执行（会 DROP DATABASE 后重建）
--
-- 时间戳命名约定（m44）：
--   大多数表使用 'update_time'。新表应统一使用 'update_time'。
--
-- 软删除约定（m45）：
--   im_message 使用 'status TINYINT'（0-正常，1-已撤回）而非 'deleted' 标志。
--   im_conversation 使用 'status TINYINT'（0-正常，1-已解散）。
--   im_conversation_member 使用 'status TINYINT'（0-正常，1-已退出）。
--   新表如需软删除，建议统一使用 'deleted TINYINT DEFAULT 0'。
--
-- im_message.status 说明（m43）：
--   Flyway V2 迁移已将旧数据中 status=2（已撤回）修正为 status=1（已撤回），
--   使状态值连续：0=正常，1=已撤回。本文件 COMMENT 已与迁移一致。
-- ============================================================================
...
-- ============================================================================
-- 外键约束（m46）
-- 使用 ALTER TABLE ... IF NOT EXISTS 模式，避免重复执行报错。
-- MySQL 不原生支持 IF NOT EXISTS 语法，故通过检查 information_schema 实现。
-- ============================================================================

-- im_conversation_member.conversation_id → im_conversation.id
SET @fk_exists = (
    SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = 'zxyz_im'
      AND TABLE_NAME = 'im_conversation_member'
      AND CONSTRAINT_NAME = 'fk_icm_conversation'
      AND CONSTRAINT_TYPE = 'FOREIGN KEY'
);
SET @sql = IF(@fk_exists = 0,
    'ALTER TABLE im_conversation_member ADD CONSTRAINT fk_icm_conversation FOREIGN KEY (conversation_id) REFERENCES im_conversation(id)',
    'SELECT 1'
);
...
-- im_message.conversation_id → im_conversation.id
SET @fk_exists = (
    SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = 'zxyz_im'
      AND TABLE_NAME = 'im_message'
      AND CONSTRAINT_NAME = 'fk_im_conversation'
      AND CONSTRAINT_TYPE = 'FOREIGN KEY'
);
SET @sql = IF(@fk_exists = 0,
    'ALTER TABLE im_message ADD CONSTRAINT fk_im_conversation FOREIGN KEY (conversation_id) REFERENCES im_conversation(id)',
    'SELECT 1'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
