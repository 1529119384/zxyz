...
/**
 * {@link VerifyCodeConfig} 的装配契约测试 —— 钉死「pepper 真的从 {@link ServiceProperties}
 ...
 * {@code new VerifyCodeHasher(...)}，<b>没有任何测试证明过 {@code @Bean} 装配真的能拿到 pepper</b>
 * —— 一旦 {@code ServiceProperties} 没被注册成 bean 而注入点又恰好能编译通过，
 * 症状会是「服务起不来」或「pepper 恒为 null ⇒ 构造器抛异常」，本测试把这条边钉死。</p>
 *
 * <p>生产装配路径与本测试等价：启动类 {@code ZxyzUserApplication} 带
 * {@code @ConfigurationPropertiesScan}（等价于这里的 {@code @EnableConfigurationProperties}），
 * 本配置类落在 {@code @ComponentScan(basePackages = "uno.acloud.user")} 范围内。</p>
 *
 * <p>金标准摘要由 Python {@code hmac} 与 openssl 两套独立实现算出，不是被测代码自产。</p>
 */
class VerifyCodeConfigTest {
    ...
    @Test
    void hasherShouldBeWiredFromServiceProperties() {
        runner.withPropertyValues("app.verification.code-pepper=test-pepper")
                .run(ctx -> {
                    assertThat(ctx).hasNotFailed();
                    assertThat(ctx).hasSingleBean(VerifyCodeHasher.class);
                    // 这一步同时证明：pepper 真的绑定成功，且域分隔前缀与算法没变
    ...
    @Test
    void contextShouldFailWhenPepperIsBlank() {
        runner.withPropertyValues("app.verification.code-pepper=")
                .run(ctx -> {
                    assertThat(ctx).hasFailed();
                    assertThat(ctx.getStartupFailure())
                            .hasRootCauseInstanceOf(IllegalStateException.class);
    ...
    void contextShouldFailWhenPepperIsTemplatePlaceholder() {
        runner.withPropertyValues("app.verification.code-pepper=CHANGE_ME_JASYPT_PASSWORD")
                .run(ctx -> {
                    assertThat(ctx).hasFailed();
                    assertThat(ctx.getStartupFailure())
                            .hasRootCauseInstanceOf(IllegalStateException.class)
                            .rootCause()
                            .hasMessageContaining("CHANGE_ME_JASYPT_PASSWORD");
                });
    }
    ...
    void contextShouldFailWhenPepperIsDevPepperWithoutExplicitAllow() {
        runner.withPropertyValues("app.verification.code-pepper=dev-only-insecure-pepper")
                .run(ctx -> {
                    assertThat(ctx).hasFailed();
                    assertThat(ctx.getStartupFailure())
                            .hasRootCauseInstanceOf(IllegalStateException.class)
    ...
    /** 显式置 {@code allow-insecure-pepper=true}（dev / test profile 才应如此）后，dev pepper 才被接受。 */
    @Test
    void contextShouldStartWhenDevPepperExplicitlyAllowed() {
        runner.withPropertyValues(
                        "app.verification.code-pepper=dev-only-insecure-pepper",
                        "app.verification.allow-insecure-pepper=true")
                .run(ctx -> {
                    assertThat(ctx).hasNotFailed();
                    assertThat(ctx).hasSingleBean(VerifyCodeHasher.class);
                    assertThat(ctx.getBean(VerifyCodeHasher.class).hash("123456"))
