...
- **密码来源**（**2026-09-14 起改为「必须显式提供」**）：
  - 配置了 `app.admin.bootstrap.password`（部署变量 `ADMIN_INIT_PASSWORD`）→ 使用该密码，**不写入日志**。
  - 未配置（留空）→ **拒绝创建该账号**，并打印一条含指引的 ERROR（应用照常启动）。
    历史上会自动生成 16 位随机密码并以 warn 级别打印明文，**该行为已删除**：
    容器日志经 promtail 进入 Loki ⇒ 明文口令等于长期留存于日志系统，
    且「口令只存在于日志中」意味着没有任何人真正持有一个可管理的凭据。
  - 提供口令的方式二选一（改完重启 user-service 即可）：
    ```bash
    # ① 自动生成（写入 .env，权限 600）
    bash scripts/init-secrets.sh
    # ② 手工设置
    #   在 .env 中把 ADMIN_INIT_PASSWORD 设为高强度口令
    ```
  - ⚠️ 该账号**已存在**时本项完全不参与（幂等跳过）⇒ **已上线环境不需要补配**。
...
| `ADMIN_INIT_PASSWORD` | 空 | 初始管理员密码；**留空时拒绝创建账号**（不再随机生成、不再打印日志）。映射到 `app.admin.bootstrap.password` |

**安全建议**：生产环境**务必**在部署前于 `.env` 中显式设置 `ADMIN_INIT_PASSWORD` 为高强度密码（或直接跑 `scripts/init-secrets.sh` 自动生成）；无论何种方式创建，请登录后立即修改密码。`scripts/validate-env.sh` 会在该项为空时给出 WARN。
...
| `NACOS_USERNAME` | `nacos` | Nacos 登录用户名（**控制台与 `import.sh` 使用**；应用服务客户端不带凭证，见下方说明） |
| `NACOS_PASSWORD` | `CHANGE_ME_NACOS_PASSWORD` | Nacos 管理员密码（生产环境必须修改）。**必须**由 `scripts/init-nacos-auth.sh` 落库到 `nacos.users`，否则用户表为空 ⇒ 控制台登不进、token 通道恒失败 |
...
#### Nacos 鉴权现状与未来启用（迁移 checklist）

> ⚠️ **严禁在应用客户端为空凭证时开启 `nacos.core.auth.enabled=true`**：所有服务将无法注册/拉取配置，系统整体不可用。

**当前形态（2026-09-13 实测订正）**：`nacos.core.auth.enabled=false`、`admin.enabled=true`、`console.enabled=true`。
应用客户端以**空凭证**运行是**刻意**的（各服务 yml 的 `${NACOS_USERNAME:}` 默认为空；nacos-client 在 username 为空时跳过 HTTP login）。

**订正上一版的原因描述**：此前写作「login 端点被 Spring 层 403 拦截（与凭证对错无关）」，实测结论不同 ——
Nacos 3.x 把鉴权端点**只暴露在 console 端口**（容器 `8080` / 宿主 `127.0.0.1:18081`，且路径**无 `/nacos` 前缀**），而应用客户端的 `server-addr` 指向 **8848**；
8848 上的 `/nacos/v3/auth/**` 一律 **403**（不是凭证对错，而是**该端口本就不提供 auth**）。另：`/nacos/v1/auth/login` 在 Nacos 3.x **已彻底下线**（404），失败后**不会**自动回退 v3。

**这不影响本仓库的发布链路**：`nacos-config/import.sh` 已改为**在 console 端口登录**（`POST http://<console>/v3/auth/user/login`）并以 server-identity 头兜底，
因此配置导入不受上述限制（见 `nacos-config/README.md`）。

**若将来要开启 `nacos.core.auth.enabled=true`，按以下顺序**：

1. 确认 `nacos.users` 非空（否则开启后无人能登录）：在服务器执行 `./scripts/init-nacos-auth.sh`。
2. **先在小范围验证「客户端开启鉴权后能否拿到 token」** —— ⚠️ **本仓未验证**：auth 端点在 console 端口而客户端 `server-addr` 指向 8848，两者的兼容路径尚不明确。
   **验证通过前不要开启 `auth.enabled`。**
3. 回填 11 处 yml 凭证（10 个服务 `application.yml` + `zxyz-common` 的 `application-common.yml`：`${NACOS_USERNAME:}` → 恢复 env 引用与默认值）。
4. 恢复 `docker-compose.yml` 中 10 个应用服务的 `NACOS_USERNAME`/`NACOS_PASSWORD` 注入。
